...
export const redeemPositionsAuto = async () => {
    ...
    console.log(`\n[SUCCESS] Connected to Polygon RPC`);
    ...
    if (allPositions.length === 0) {
        console.log('\n[INFO] No open positions detected for proxy wallet.');
        return;
    }
    ...
    console.log(`   [RESOLVED]: ${redeemablePositions.length}`);
    ...
    console.log(`[WARNING]: Each redemption requires gas fees on Polygon`);
    ...
    console.log(
        `\n[INFO] Grouped into ${positionsByCondition.size} unique conditions`
    );
    ...
    for (const [conditionId, positions] of positionsByCondition.entries()) {
        ...
        positions.forEach((pos, idx) => {
            const status = pos.curPrice >= RESOLVED_HIGH ? '[WIN]' : '[LOSS]';
            console.log(
        ...
        if (conditionIndex < positionsByCondition.size) {
            console.log(`   [WAITING] Waiting 2s before next transaction...`);
            await new Promise((resolve) => setTimeout(resolve, 2000));
        }
    }

    console.log('\n════════════════════════════════════════════════════');
    console.log('[SUMMARY] Position redemption');
...
if (require.main === module) {
    redeemPositionsAuto()
        .then(() => process.exit(0))
        .catch((error) => {
            console.error('[ERROR] Script aborted due to error:', error);
            process.exit(1);
        });
}
